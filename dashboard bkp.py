...
def create_dash_app(flask_app, url_base_pathname='/dashboard/'):
            ...
            content = html.Div([
                ...
                html.Div([
                    ...
                    html.Div([
                        ...
                        html.Div([
                            dbc.Button(
                                [html.I(className="bi bi-layers me-1"), "Med. complementaria"],
                                id='med-comple-button',
                                color='info',
                                external_link=True,
                                href='/dashboard_med_comple/',
                                style={
                    ...
                    dbc.Tooltip("Ir a Medicina complementaria", target='med-comple-button', placement='bottom', style={'zIndex': 9999}),
    ...
    @dash_app.callback(
        Output('med-comple-button', 'href'),
        Input('url', 'pathname')
    )
    def update_med_link(pathname):
        if not pathname:
            return '/dashboard_med_comple/'
        base_segment = url_base_pathname.strip('/')
        segments = [seg for seg in pathname.strip('/').split('/') if seg]
        if len(segments) >= 2 and segments[0] == base_segment:
            return f"/dashboard_med_comple/?codcas={segments[-1]}"
        return '/dashboard_med_comple/'
