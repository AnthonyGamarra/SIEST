    ...
    html.Div([
        html.Div([
            html.Div([
                ...
                html.H2(
                    f"Detalle de Atenciones - Prioridad 3",
                    style={
                        'color': BRAND,
                        'fontFamily': FONT_FAMILY,
                        'fontSize': '26px',
        ...
        html.Div([
            ...
            dcc.Download(id="download-csv-3")
        ], style={
            'display': 'flex',
            'alignItems': 'center',
...
def layout(codcas=None, **kwargs):
    return html.Div([
        ...
        html.Div([
            ...
            dcc.Loading(dcc.Graph(id="timeline-atenciones-3")),
        ], style={
            "backgroundColor": CARD_BG,
            "borderRadius": "14px",
            "boxShadow": "0 8px 20px rgba(0,0,0,0.08)",
        ...
        html.Div([
            ...
            dcc.Loading(dcc.Graph(id="pie-tipo-paciente-3"))
        ], style={
            "backgroundColor": CARD_BG,
            "borderRadius": "14px",
            "boxShadow": "0 8px 20px rgba(0,0,0,0.08)",
...
def update_page_content(codcas, search):
    ...
    query = f"""
        ...
        FROM (
            SELECT 
                ROW_NUMBER() OVER (PARTITION BY cod_centro, cod_estandar, acto_med,cod_emergencia ORDER BY cast(secuen_aten as integer) asc) AS SECUENCIA, c.*
            FROM (SELECT
                    a.cod_centro, 
                    a.periodo, 
                    a.cod_topico,
                    ...
                    a.cod_diagnostico,
                    dg.diagdes,
                    (case when a.cod_estandar = '04' then '2'
                    else (case when a.cod_prioridad='2' then '2'
    ...
    try:
        df_fecha = df.copy()
        df_fecha['fecha_aten'] = pd.to_datetime(df_fecha['fecha_aten'], errors='coerce', dayfirst=True)
        timeline_df = (
...
def register_callbacks(app):
    ...
    app.callback(
        Output("download-csv-3", "data"),
        [Input("download-csv-btn-3", "n_clicks"),
         Input("ate-topicos-codcas-store-3", "data"),
         Input("ate-topicos-url-3", "search")],
        prevent_initial_call=True
    )(download_csv)
