...
color_config = {'gradient': 'linear-gradient(135deg, #28a745 0%, #218838 100%)', 'icon': 'bi-check-circle-fill', 'bg': '#28a745'}
    ...
    html.Div([
        html.Div([
            html.Div([
                ...
                html.H2(
                    f"Detalle de Atenciones - Prioridad 4",
                    style={
                        'color': BRAND,
                        'fontFamily': FONT_FAMILY,
                        'fontSize': '26px',
         ...
         html.Div([
            ...
            dcc.Download(id="download-csv-4")
        ], style={
            'display': 'flex',
            'alignItems': 'center',
...
def update_page_content(codcas, search):
    ...
    if not periodo or not anio_str:
        return (
            empty_fig("Top 10 Diagnósticos (Prioridad 4)"),
    ...
    try:
        ...
        fig = empty_fig("Top 10 Diagnósticos (Prioridad 4)")
...
def register_callbacks(app):
    ...
    app.callback(
        Output("download-csv-4", "data"),
        [Input("download-csv-btn-4", "n_clicks"),
         Input("ate-topicos-codcas-store-4", "data"),
         Input("ate-topicos-url-4", "search")],
        prevent_initial_call=True
    )(download_csv)
...
def download_csv(n_clicks, codcas, search):
    ...
    return dcc.send_data_frame(
        df.to_csv,
        filename=f"atenciones_{codcas}_{anio_str}_{periodo}_prioridad_4.csv",
        index=False
    )
