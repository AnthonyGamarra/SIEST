...
def layout(codcas=None, **kwargs):
    return html.Div([
        ...
        html.Div([
            ...
            dcc.Loading(
                dcc.Graph(id="timeline-atenciones-1")
            )
        ], style={
            "backgroundColor": CARD_BG,
            "borderRadius": "14px",
        ...
        html.Div([
            ...
            dcc.Loading(dcc.Graph(id="pie-tipo-paciente-1"))
        ], style={
            "backgroundColor": CARD_BG,
            "borderRadius": "14px",
            "boxShadow": "0 8px 20px rgba(0,0,0,0.08)",
...
# --- REGISTRO DE CALLBACKS PARA DASH PRINCIPAL ---
def register_callbacks(app):
    ...
    def update_page_content(codcas, search):
        ...
        try:
            diag_df = (
                df.groupby('diagdes', dropna=False)
                .size()
        ...
        try:
            ...
            if timeline_df.empty:
                ...
                timeline_fig.update_traces(marker_line=dict(color='rgba(0,0,0,0.08)', width=1), texttemplate='%{text:,.0f}', textposition='outside')
    ...
    def download_csv(n_clicks, codcas, search):
        ...
        return dcc.send_data_frame(
            df.to_csv,
            filename=f"atenciones_{codcas}_{anio_str}_{periodo}_prioridad_1.csv",
            index=False
        )
