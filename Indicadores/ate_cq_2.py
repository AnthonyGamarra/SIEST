...
N                   = 2
COMPLEJIDAD_LABEL   = "Complejidad B"
COMPLEJIDAD_COLOR   = "#fd7e14"
COMPLEJIDAD_FILTER  = "AND cq.cod_complejidad = 'B'"
CSV_SUFFIX          = "complejidad_B"
...
def layout(codcas=None, **kwargs):
    return html.Div([
        ...
        html.Div([
            html.Div([
                html.Div([
                    html.H2(f"Detalle {COMPLEJIDAD_LABEL}", style={
                        "color": BRAND, "fontFamily": FONT_FAMILY,
                        "fontSize": "22px", "margin": "0", "fontWeight": "700"
                    }),
                ], style={"display": "flex", "alignItems": "center"}),
                html.P(f"Fecha {date.today().strftime('%d/%m/%Y')} | Centro Quirúrgico",
...
def register_callbacks(app):
    ...
    def update_content(codcas_enc, search):
        ...
        if engine is None:
            return html.P("Error de conexión.", style={"color": "#dc3545", "padding": "40px"})
        ...
        area = (df.groupby("area", dropna=False).size()
                  .reset_index(name="Cantidad")
                  .sort_values("Cantidad", ascending=False).head(10))
        area["area"] = area["area"].fillna("Sin area")
        ...
        # ---- AG Grid ----
        cols_table = ["cenasides", "area", "servicio", "cpms", "cod_complejidad",
    ...
    def download_csv(n_clicks, codcas_enc, search):
        ...
        return _dcc.send_data_frame(
            df.to_csv,
            f"cq_{CSV_SUFFIX}_{codcas}_{anio_str}_{periodo}.csv",
            index=False,
        )
